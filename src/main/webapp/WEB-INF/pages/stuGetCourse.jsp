            ...
            <table class="am-table am-table-striped am-table-hover table-main">
                ...
                <c:forEach var = "course" items="${courses}" >
                    <%
                        ...
                        List<Integer> courseCno = (List<Integer>) session.getAttribute("courseCno");
                        int isSelect = courseCno.contains(course.getCno())?1:0;
                        pageContext.setAttribute("isSelect",isSelect);
                    %>
                    ...
                    <tr>
                        ...
                        <td>${course.teacher.department.departName}</td>
                        <td>星期${course.weekend}第${course.times.time_id}大节</td>
                        <td>${course.csname}</td>
                        <td>${course.cscore}</td>
                        <td>
                                ...
                                <div class="am-btn-group am-btn-group-xs">
                                    <c:choose>
                                        <c:when test="${isSelect == 0}">
                                            <input type="button" value="选课" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="addCourse('${stuinfo.sno}','${course.cno}')"/>
                                        </c:when>
                                        <c:when test="${isSelect == 1}">
                                            <input type="button" value="退课" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="deleteCourse('${stuinfo.sno}','${course.cno}')"/>
                                        </c:when>
                                    </c:choose>
                                </div>
                            </div>
